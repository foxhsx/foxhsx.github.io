...
if (localStorage.hasOwnProperty('PaperColorsCache')) {
    try {
        colorsCache = JSON.parse(localStorage.getItem('PaperColorsCache'));
    }
    catch (e) {
        colorsCache = {};
    }
...
async function getColor(key: string, hash: string, imageURL: string) {
    ...
    if (!colorsCache.hasOwnProperty(key) || colorsCache[key].hash !== hash) {
        ...
        localStorage.setItem('PaperColorsCache', JSON.stringify(colorsCache));
    }
